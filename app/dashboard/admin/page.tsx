...
import { Building2, Plus, Search, Settings2, MapPin, Loader2, ShieldAlert } from 'lucide-react';
...
export default function SuperAdminPage() {
  const [copros, setCopros] = useState<Copropriete[]>([]);
  ...
  const fetchCopros = useCallback(async () => {
    const { data } = await supabase.from('coproprietes').select('*').order('created_at', { ascending: false });
  ...
  async function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      toast.error("Erreur lors de la creation de l'espace");
    } else {
      toast.success('Nouvelle Copropriete initialisee !');
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-3">
              <Badge className="bg-rose-500/20 text-rose-400 border-none px-4 py-1.5 uppercase tracking-widest font-black text-[10px]">
                <ShieldAlert className="h-3 w-3 mr-2 inline" /> Acces Restreint : Super Admin
              </Badge>
              <h1 className="text-4xl md:text-5xl font-black tracking-tighter">Gestion du Parc</h1>
              <p className="text-slate-400 font-medium max-w-xl">
                Controlez les instances de CoproSync. Ajoutez de nouveaux clients et gerez l&apos;infrastructure globale.
              </p>
            </div>
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 sm:max-w-[500px]">
                <DialogHeader className="mb-6">
                  ...
                  <p className="text-slate-500 font-medium">Deployez un espace vierge pour une nouvelle residence.</p>
                </DialogHeader>
                ...
                <form onSubmit={handleCreate} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Input
                      name="nom"
                      placeholder="Ex: Les Jardins de l'Olympe"
                      required
                      className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-lg focus:ring-4 focus:ring-indigo-50"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-xs font-black uppercase text-slate-400 tracking-widest ml-1">Adresse Complete</Label>
                    <Input
                      name="adresse"
                      placeholder="15 avenue des Champs..."
                      required
                      className="h-14 rounded-2xl bg-slate-50 border-none font-bold focus:ring-4 focus:ring-indigo-50"
                    />
                  </div>
                  <Button
                    type="submit"
                    disabled={isCreating}
                    className="w-full h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 font-black text-lg text-white shadow-xl shadow-indigo-100 mt-4"
                  >
                    {isCreating ? <Loader2 className="animate-spin h-6 w-6" /> : 'Deployer la Copropriete'}
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
              ...
              <p className="text-slate-400 font-bold italic">Aucune instance ne correspond a la recherche.</p>
            </div>
          ) : (
            filteredCopros.map((copro) => (
              <Card
                key={copro.id}
                className="rounded-[2.5rem] border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-300 bg-white group overflow-hidden"
              >
                <CardHeader className="p-8 pb-4">
                  ...
                  <CardTitle className="text-2xl font-black text-slate-900 leading-tight">{copro.nom}</CardTitle>
                </CardHeader>

                <CardContent className="p-8 pt-0 space-y-6">
                  <div className="flex items-center gap-3 text-slate-500 font-medium text-sm">
                    <MapPin className="h-4 w-4 shrink-0 text-slate-300" />
                    <span className="line-clamp-1">{copro.adresse || 'Adresse non renseignee'}</span>
                  </div>
                  ...
                  <div className="pt-6 border-t border-slate-50 grid grid-cols-2 gap-4">
                    ...
                    <Link href={`/dashboard/admin/${copro.id}`}>
                      <Button
                        variant="outline"
                        className="h-auto rounded-2xl border-slate-200 text-indigo-600 font-bold hover:bg-indigo-50 hover:border-indigo-200 transition-colors"
                      >
                        <Settings2 className="h-4 w-4 mr-2" /> Gerer
                      </Button>
                    </Link>
                  </div>
                </CardContent>
